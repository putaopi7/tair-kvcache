...
ErrorCode BatchGetLocationErrorStub(
    void *, RequestContext *, const KeyVector &, const BlockMask &, std::vector<CacheLocationMap> &) {
    return EC_ERROR;
}
...
public:
    ...
    std::unique_ptr<CacheGarbageCollector> MakeGc(const CacheGarbageCollector::Config &config) {
        return std::make_unique<CacheGarbageCollector>(
            config, registry_manager_, meta_indexer_manager_, executor_, metrics_registry_);
    }
...
TEST_F(CacheGarbageCollectorTest, FixedPredicateIsFailClosedAndRequestIsBounded) {
    ...
    ASSERT_TRUE(request.expected_status.has_value());
    EXPECT_EQ(CLS_WRITING, request.expected_status.value());
...
TEST_F(CacheGarbageCollectorTest, TickScansOneBatchAndSubmitsConditionalAsyncDelete) {
    ...
    ASSERT_TRUE(submitted_requests.front().expected_status.has_value());
    EXPECT_EQ(CLS_WRITING, submitted_requests.front().expected_status.value());
...
TEST_F(CacheGarbageCollectorTest, RealExecutorBacklogDoesNotBlockGcSubmitAndStopsFurtherScan) {
    ...
    stub_.set(ADDR(MetaSearcher, BatchGetLocation), BatchGetLocationErrorStub);
