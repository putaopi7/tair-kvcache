...
void CacheManager::RollbackAddLocations(RequestContext *request_context,
    ...
    if (!uncertain_keys.empty()) {
        ...
        if (!meta_searcher) {
            ...
            meta_searcher->BatchDeleteLocation(request_context,
                                               uncertain_keys,
                                               uncertain_location_ids,
                                               delete_results,
            ...
            for (size_t i = 0; i < uncertain_indices.size(); ++i) {
                if (i < delete_results.size() && delete_results[i] == EC_OK) {
            ...
            for (size_t i = 0; i < uncertain_indices.size(); ++i) {
                if (i >= delete_results.size()) {
                    continue;
                }
                if (delete_results[i] == EC_NOENT || (delete_results[i] == EC_OK && metadata_synced)) {
