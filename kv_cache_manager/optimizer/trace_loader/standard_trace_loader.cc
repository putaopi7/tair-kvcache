...
StandardTraceLoader::ParseLine(const std::string &line, const std::function<void(const std::string &)> &fail) {
...
void StandardTraceLoader::StreamFromFile(const std::string &trace_file_path, const TraceCallback &on_trace) {
    std::ifstream file(trace_file_path);
    ...
    KVCM_LOG_INFO("Streamed %ld traces from file: %s", static_cast<long>(emitted), trace_file_path.c_str());
}
